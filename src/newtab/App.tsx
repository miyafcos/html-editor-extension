...
import type { NewTabIndexEntry, ReportEntry, Settings, UndoSnapshot } from "../reporthub/types";
  ...
  return (
    ...
    >
      ...
      <span className="row-copy">
        ...
        <small>
          {entry.group} <span aria-hidden="true">·</span> {locationLabel(entry)}
        </small>
      </span>
  ...
  return (
    <main className="hub-shell" data-testid="hub-shell" data-ready={settings ? "true" : "false"}>
      ...
      {index.length === 0 && openTabs.length === 0 ? (
        ...
        <section className="kind-grid">
          {KINDS.map((kind) => (
            <section className={`kind-column kind-column-${kind}`} key={kind} data-kind={kind}>
              <h2>
                <span aria-hidden="true">{kindIcon(kind)}</span> {S.kind[kind]}
              </h2>
              {BANDS.map((band) => {
                const rows = merged.filter((entry) => entry.kind === kind && visibleByBand(entry, band));
                return (
                  <section className="band" key={band} data-testid={`band-${kind}-${band}`}>
                    <h3>{S.band[band]}</h3>
                    <div className="band-rows">
                      {rows.length ? (
                        ...
                        <p className="empty-band">{S.empty[band]}</p>
                      )}
                    </div>
                  </section>
                );
